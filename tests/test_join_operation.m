...
Value_Params.alpha = 1.0; % 每小时飞行能耗（用于能量可行性/utility 成本模型）
Value_Params.beta = 1.0;  % 每小时执行能耗
...
for j = 1:Value_Params.M
    ...
    tasks(j).duration = 0; % 默认不给执行时长，避免影响其它用例
end
...
% ------------------ Case 4: 资源超额 -> 可行性检测应强制拒绝 ------------------
fprintf('Case4: capacity exceeded should be rejected (feasibility gate)\n');
Value_Params.preferenceFcn = @(tasks_, agents_, before_, after_, agentID_, params_, vd_, target_) 1;

Value_data4 = Value_data;
% 预先占用资源类型1（让后续再加入一次会超额）
Value_data4.resources_matrix = zeros(Value_Params.M, Value_Params.K);
Value_data4.resources_matrix(1, 1) = agents(1).resources(1); % 已经把 r=1 全部用在任务1
Value_data4.coalitionstru = zeros(Value_Params.M + 1, Value_Params.N);
Value_data4.coalitionstru(1, 1) = 1;
...
assert(sum(vd4.resources_matrix(:, 1)) <= agents(1).resources(1) + 1e-9, 'Case4 failed: capacity constraint violated');
disp('Case4 passed');
...
agents5 = agents;
agents5(1).energy = 100; % 总能量预算（单位与模型一致：t_wait_total*alpha + T_exec_total*beta）
...
tasks5(2).priority = 2;
tasks5(3).priority = 3;
tasks5(1).duration = 10;
tasks5(2).duration = 10;
tasks5(3).duration = 10;
