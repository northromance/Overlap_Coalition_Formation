...
% HUO2025_MAIN 基于多智能体联盟形成与任务值估计的主算法函�?
%
% 核心逻辑�?
...
%   4. 观测与更�?(Collect & Update)：智能体执行任务后获得观测值，利用贝叶斯推断更新信念�?
%   5. 评估：计算每轮的联盟效用、成本和净收益，统计演化曲线�?
%
% 输入�?
%   agents       - 智能体结构体数组 (位置、速度、资源、传感器参数�?
%   tasks        - 任务结构体数�?(位置、真实价值、资源需求等)
%   AddPara      - 附加参数 (保留接口)
%   Value_Params - 全局参数 (M, N, K, 轮数�?
%
% 输出�?
%   Value_data   - 最终时刻的系统状�?(联盟结构、资源分配、总效�?
%   history_data - 历史记录 (每轮的收益、成本曲线，用于画图)

%% ==================== 0. 随机数种子设置（确保可复现性）====================
% 修复：在算法开始时设置随机数种子，确保结果可复�?
...
total_value_history = zeros(1, Value_Params.num_rounds);  % 记录每轮完成的总任务价�?
total_value_possible = sum(arrayfun(@(t) t.value, tasks));% 任务总潜在价�?(上限)

% 初始状态：所有智能体都在 Void 任务 (�?M+1 个任�? 中待�?
...
for counter = 1:Value_Params.num_rounds
    ...
    while(doneflag == 0)
        ...
        % 每个智能体基于当前收益计算，贪婪地选择最佳任�?
        for ii = 1:Value_Params.N
            % 调用 Value_order：计算如果加入其他任务能否提高效�?
    ...
    % --- 观测与信念更�?(Observation & Belief Update) ---
    % 联盟稳定后，智能体执行任务并获得观测�?
    ...
    % 遍历每个智能�?根据实际的需�?计算联盟效用 用来存储 跟上面用信念决策不同
    ...
    counter = counter + 1; % 似乎多余，for 循环会自动增�?
