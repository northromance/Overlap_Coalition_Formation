classdef UtilityEvaluator
    ...
    methods(Static)
        ...
        function [agentutility, task_utilities] = calc_agent_total_utility(SC, agents, tasks, Value_Params, Value_data, AddPara)
            ...
            %   其中：
            %     Val(A_m)    = E[V_m] * D_m        % 期望价值 * 完成度
            %     Cost(A_m)   = sum_{i in C_m} r_{i,m} * C_i   % 联盟加权成本
            %     r_{i,m}     = ||s_{i,m}|| / sum_j ||s_{j,m}||  % 资源贡献比例
            %     C_i         = alpha_fly*t_fly + alpha_wait*t_wait + beta*t_exec  % 智能体 i 的总路径成本
            %     rho_m       = tasks(m).priority   % 任务重要性缩放因子
            %
            %   智能体 n 的总效用：
            %     agentutility = sum_{m in M_n} r_{n,m} * U_m(A_m)
            %
            % 输入：
            %   SC           - 全局联盟结构 (M x 1 Cell，每格为 N x K 资源分配矩阵)
            %   agents       - 智能体结构体数组
            %   tasks        - 任务结构体数组
            %   Value_Params - 全局参数 (N, M, K, task_type, task_type_demands)
            %   Value_data   - 当前智能体数据 (agentID, initbelief)
            %   AddPara      - (可选) 附加参数，可含 resource_confidence
            %
            % 输出：
