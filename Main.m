...
WORLD.value=[800,1000,1500];    % 任务价值候选集（调整长度即可改变任务类型数量）
...
num_task_types = numel(WORLD.value); % 任务类型数量（与价值集合长度一致）
...
for j = 1:M    
    ...
    tasks(j).value = WORLD.value(tasks(j).type);                           % 类型决定价值（按 WORLD.value 索引）
