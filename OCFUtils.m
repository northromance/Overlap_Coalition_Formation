classdef OCFUtils
    ...
    methods (Static)
        ...
        function [SC_P, SC_Q, R_agent_P, R_agent_Q] = calc_move_changes(Value_data, agents, Value_Params, cur_task_idx, target_task_idx, agent_col_idx)
            % CALC_MOVE_CHANGES 计算智能体在任务迁移时的资源状态变化
            %
            % 核心修正：
            %   对于 R_agent (个体资源矩阵)，采用“全盘清零”策略，防止历史脏数据残留。
            ...
            %% 3. [关键修改] 个体资源矩阵：全盘清零
            % 不管之前在哪，先制造一个全空的资源矩阵。
            % 这样保证了 R_agent_Q 里永远只有目标任务那一行有数据。
            R_agent_Q = zeros(M, K);

            %% 4. 执行“撤出”逻辑 (只针对 SC 全局矩阵)
            % 个体矩阵 R_agent_Q 已经在上面清零了，这里只需要处理 SC
            if cur_task_idx <= M
                % 全局视图：从旧任务的小组中移除自己
            ...
            %% 5. 执行“加入”逻辑 (针对 SC 和 R_agent)
            if target_task_idx <= M
                % A. 个体视图：填入新任务 (因为其他行全是0，所以这里是唯一的非零行)
