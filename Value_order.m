function [incremental,curnumberrow,Value_data]=Value_order(agents, tasks, Value_data, Value_Params,counter)
%用于判断联盟结构是否改变的标志位
...
curagentutility=Value_utility(agents, tasks,curnumberrow,curnumbercolumn, curnumberofcoworker,Value_data,Value_Params);%计算agent当前效用
...
for j=1:Value_Params.M+1
    ...
    candidateagentutility(j)=Value_utility(agents, tasks, j, Value_data.agentID, candidatenumberofcoworker,Value_data,Value_Params);%调用utility函数
    Value_data.agentID;
end
