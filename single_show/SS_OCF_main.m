function [Value_data, history_data] = SS_OCF_main(agents, tasks, AddPara, Value_Params)
...
Value_data = SS_WorldSim.init_value_data(agents, tasks, Value_Params);
[Value_data, summatrix] = SS_WorldSim.init_observe_belief_neighbor(Value_data, Value_Params.N, Value_Params.M, Value_Params);
    ...
    if counter == 1
        ...
        for i = 1:Value_Params.N
            ...
            for k = 1:Value_Params.K % 遍历每一类资源
                ...
                % 按概率选择任务（封装在 SS_OCFUtils.sample_task_from_probs 中）
                selected_task = SS_OCFUtils.sample_task_from_probs(probs(k, :), Value_Params.M);
                ...
                expected_demand = SS_WorldSim.estimate_demand_from_belief( ...
        ...
        for i = 1:Value_Params.N
            Value_data(i).SC = SC_global;
            Value_data(i).resources_matrix = SS_OCFUtils.get_agent_resource_matrix(SC_global, i, Value_Params);
            Value_data(i).coalitionstru = SS_OCFUtils.build_coalitionstru_from_SC(SC_global, Value_Params, agents);
        end
    ...
    while(doneflag == 0)
        ...
        for ii = 1:Value_Params.N
            ...
            if accept
                ...
                new_coalitionstru = SS_OCFUtils.build_coalitionstru_from_SC(SC_candidate, Value_Params, agents);
                ...
                Value_data(ii).resources_matrix = SS_OCFUtils.get_agent_resource_matrix(SC_candidate, ii, Value_Params);
        ...
        for ii = 1:Value_Params.N
            ...
            Value_data(ii).resources_matrix = SS_OCFUtils.get_agent_resource_matrix(final_SC, ii, Value_Params);
        end

        current_utility_global = 0;
        for j = 1:Value_Params.N
            current_utility_global = current_utility_global + SS_UtilityEvaluator.calc_agent_total_utility(final_SC, agents, tasks, Value_Params, Value_data(j), AddPara);
        end
...
if cleaned_entries > 0
    cleaned_coalitionstru = SS_OCFUtils.build_coalitionstru_from_SC(SC_clean, Value_Params, agents);
    for agent_idx = 1:N
        Value_data(agent_idx).SC = SC_clean;
        Value_data(agent_idx).coalitionstru = cleaned_coalitionstru;
        Value_data(agent_idx).resources_matrix = SS_OCFUtils.get_agent_resource_matrix(SC_clean, agent_idx, Value_Params);
    end
end
end
...
for k = 1:K
    ...
    % 按概率选择目标任务（封装在 SS_OCFUtils.sample_task_from_probs 中）
    selected_task = SS_OCFUtils.sample_task_from_probs(probs(k, :), M);
    if isempty(selected_task), continue; end
    % 已经对该任务投放过该类资源则跳过
...
function GSU = calculate_local_social_utility(SC_old, SC_candidate, agent_idx, agents, tasks, Value_Params, Value_data, AddPara) %#ok<INUSL>
    ...
    for j = 1:Value_Params.N
        ...
        GSU = GSU + SS_UtilityEvaluator.calc_agent_total_utility(SC_candidate, agents, tasks, Value_Params, temp_data, AddPara);
    end
end
...
function [delta_E, GSU_candidate, GSU_current] = global_utility_diff(tasks, agents, SC_current, SC_candidate, agent_idx, Value_Params, Value_data_i, precomputed_GSU_current) %#ok<INUSL>
    ...
    for j = 1:Value_Params.N
        ...
        temp_data.initbelief = agent_belief;
        temp_data.SC = SC_candidate;
        GSU_candidate = GSU_candidate + SS_UtilityEvaluator.calc_agent_total_utility(SC_candidate, agents, tasks, Value_Params, temp_data, AddPara_silent);
    end
    ...
    if ~isempty(precomputed_GSU_current)
        ...
        for j = 1:Value_Params.N
            ...
            GSU_current = GSU_current + SS_UtilityEvaluator.calc_agent_total_utility(SC_current, agents, tasks, Value_Params, temp_data, AddPara_silent);
        end
    end
