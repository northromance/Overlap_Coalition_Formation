...
%   Value_data: 包含信念(belief)等信息，用于估算队友效用
    ...
    rows_n_Q = SS_OCFUtils.get_agent_tasks_fast(SC_Q, agentID);
    rows_n_P = SS_OCFUtils.get_agent_tasks_fast(SC_P, agentID);
    ...
    u_n_Q = SS_UtilityEvaluator.calc_agent_total_utility(SC_Q, agents, tasks, Value_Params, Value_data, AddPara);
    u_n_P = SS_UtilityEvaluator.calc_agent_total_utility(SC_P, agents, tasks, Value_Params, Value_data, AddPara);
    ...
    for idx = 1:length(new_tasks)
        ...
        % 获取该任务在新状态 Q 中的队友 (g)
        members = SS_OCFUtils.get_participants(SC_Q, task_id, 1e-6);
    ...
    for idx = 1:length(source_tasks)
        ...
        members = SS_OCFUtils.get_participants(SC_P, task_id, 1e-6);
...
function u = get_agent_util_proxy(target_id, target_belief, SC, agents, tasks, params, AddPara)
    ...
    u = SS_UtilityEvaluator.calc_agent_total_utility(SC, agents, tasks, params, temp_data, AddPara);
end