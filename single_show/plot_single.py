...
SEARCH_DIRS = [
    os.path.join(SCRIPT_DIR, 'results'),          # single_show/results/*.mat（优先）
...
def get_agent_field(struct_dict, field, idx):
    ...
    mat73 加载规则：
      - struct array 大小 1×N（N>1）：field → ndarray of N items（或 list）
      - struct array 大小 1×1（N=1）：field → 值本身（不包在 list/array 中）
    """
    ...
    if isinstance(val, np.ndarray):
        ...
        # 1D array with N>1 elements: mat73 loads scalar fields as 1D ndarray
        if val.ndim == 1 and val.size > 1:
            return val[idx] if idx < val.size else None
        # 2D array (e.g. resources stored as N×K or K×N)
        if val.ndim == 2 and val.shape[0] > 1:
            return val[idx] if idx < val.shape[0] else None
...
def struct_field_list(struct_data, field):
    ...
    if isinstance(struct_data, dict):
        ...
        # mat73: struct array scalar fields are loaded as 1D float64 ndarray
...
def get_struct_length(struct_data):
    ...
    if isinstance(struct_data, dict):
        ...
        for value in struct_data.values():
            ...
            elif isinstance(value, np.ndarray) and value.ndim == 1 and value.size > 1:
                max_len = max(max_len, value.size)
