    ...
    methods (Static)
        ...
        function t_exec = calc_coalition_exec_time(SC, task_idx, task, Value_Params, tol)
            ...
            for i = 1:Value_Params.N
                if any(alloc(i, :) > tol)
                    exec_times = [exec_times, SS_WorldSim.calc_exec_time(task, alloc(i, :), Value_Params, tol)]; %#ok<AGROW>
                end
            end
        ...
        function all_agents_results = calc_all_agents_with_global_sync(agents, tasks, Value_Params, SC, tol)
            ...
            all_tasks = 1:M;
            global_order = SS_OCFUtils.sort_tasks_by_priority(all_tasks, tasks);
            ...
            for order_idx = 1:M
                ...
                t_coalition = SS_WorldSim.calc_coalition_exec_time(SC, task_id, tasks(task_id), Value_Params, tol);
            ...
            for agentIdx = 1:N
                % 获取该智能体的任务列表并排序
                task_list = SS_OCFUtils.get_agent_tasks_fast(SC, agentIdx, tol);
                ...
                myOrderedTasks = SS_OCFUtils.sort_tasks_by_priority(task_list, tasks);
                ...
                for ii = 1:num_my_tasks
                    ...
                    % (3) 自身执行时长
                    my_exec_time = SS_WorldSim.calc_exec_time(tasks(task_id), R_row, Value_Params, tol);
        ...
        function demand = estimate_demand_from_belief(belief, task_type_demands, mode, confidence, rounding_mode)
            ...
            switch char(mode)
                ...
                case 'expected'
                    ...
                    demand = SS_WorldSim.apply_demand_rounding(demand, rounding_mode);
                otherwise
