...
%   probs        - (KxM 矩阵) 概率分布矩阵
    ...
    for r = 1:K
        ...
        % --- D. (可选) 调试用的判断打印 ---
        % 如果你想在运行时监控异常，可以加这段：
        check_sum = sum(probs(r, :));
        if check_sum > eps_val && abs(check_sum - 1) > 1e-5
            error('Agent %d Resource %d: Probability sum is %.8f (Should be 1)', ...
                     agentID, r, check_sum);
        end
    end
...
% 
% function probs = SA_Select_probs(Value_data, agents, tasks, Value_Params, resource_gap, current_T)
% % SA_SELECT_PROBS (修复版：增加了硬约束检查)
% %
% % 修复内容:
% %   1. 如果智能体没有某资源，该行概率直接为 0。
% %   2. 如果任务不需要某资源(得分=0)，该位置概率直接为 0 (不再是 exp(0)=1)。
% 
%     agentID = Value_data.agentID;
%     K = Value_Params.K;
%     M = Value_Params.M;
...
%     for r = 1:K
%         % 【修复1：供给端硬约束】
%         % 如果智能体根本没有第 r 种资源，那么它对所有任务的概率都必须是 0
%         if Value_data.resources(r) <= eps_val
%             probs(r, :) = 0;
%             continue; % 直接跳过后续计算，处理下一行
%         end
% 
%         scores = zeros(1, M); 
% 
...
% 
%             % --- B. 综合打分 ---
%             % 逻辑: 只要 (缺口=0 或 供给=0)，Score 就会是 0
%             score_val = (feat_prio^2 * feat_demand * feat_supply) / (feat_dist + eps_val);
% 
%             scores(j) = score_val;
%         end
% 
%         % --- C. Boltzmann 变换 (带掩码修复) ---
% 
%         % 【修复2：需求端硬约束 (Masking)】
%         % 找出所有有效的任务 (Score > 0 的任务)
%         % 如果 Score=0 (意味着不需要或无法做)，则不参与 exp 计算，概率直接锁死为 0
%         valid_mask = scores > eps_val;
% 
%         if ~any(valid_mask)
%             % 如果对于这个资源，所有任务得分都是0 (例如都没需求)，则整行概率为0
%             probs(r, :) = 0;
%             continue;
%         end
% 
...
% 
%         % Shifted Softmax (防止溢出)
%         raw_exponent = valid_scores / effective_T;
%         shifted_exponent = raw_exponent - max(raw_exponent);
% 
%         exp_values = exp(shifted_exponent);
%         sum_exp = sum(exp_values);
% 
%         % 填充概率 (只填充 valid_mask 为 true 的位置，其他保持为 0)
%         current_probs = zeros(1, M);
%         if sum_exp > eps_val
%             current_probs(valid_mask) = exp_values / sum_exp;
%         end
% 
%         probs(r, :) = current_probs;
% 
%         % --- D. 校验 (可选) ---
%         check_sum = sum(probs(r, :));
%         if check_sum > eps_val && abs(check_sum - 1) > 1e-5
%            warning('Check logic: sum is not 1');
%         end
%     end
% end