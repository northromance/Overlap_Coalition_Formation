...
for i=1:Value_Params.N
    ...
    Value_data(i).task_schedule.task_sequence = [];           % 任务执行顺序（按priority排序后的任务ID列表）
    Value_data(i).task_schedule.arrival_times = [];           % 各任务到达时间
    Value_data(i).task_schedule.start_times = [];             % 各任务开始执行时间（考虑同步等待）
    Value_data(i).task_schedule.execution_times = [];         % 各任务执行时间
    Value_data(i).task_schedule.completion_times = [];        % 各任务完成时间
    Value_data(i).task_schedule.total_flight_time = 0;        % 总飞行时间
    Value_data(i).task_schedule.total_execution_time = 0;     % 总执行时间
    Value_data(i).task_schedule.total_energy = 0;             % 总能量消耗
end
