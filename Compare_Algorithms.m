...
algorithms_to_run_ids = [3]; % 要运行的算法 ID 列表（根据 all_algorithms 中的 id 字段选择）
...
% 图形尺寸配置（[left, bottom, width, height]，单位：像素）
fig_size_main    = [100, 100, 900, 600];   % 主对比图（2行3列柱状图）
fig_size_radar   = [150, 150, 600, 450];   % 雷达图
fig_size_history = [200, 200, 400, 350];   % 历史演化图（每列宽度）
fig_size_belief  = [180, 180, 900, 600];   % 任务期望价值演化图
fig_size_alloc   = [100, 100, 900, 600];   % 资源分配图
fig_size_inner   = [100, 100, 900, 600];   % 内循环演化图（Plot_InnerLoop_Evolution）
...
num_rounds = 150;              % 迭代轮数（快速测试: 5轮）


MaxIter = 300;                      %  每轮最大迭代次数
% ========================================================================
...
SA_alpha = 0.9;                      % 降温每轮系数（轮内：T(80)≈T0×0.017，末温约2.6，接近贪婪）
SA_Tmin = 0.01;                       % 终止温度
K_stable_max = 15;                    % 稳定性阈值（连续无改进迭代次数，SA/Fang/Tabu 系列共用）

T_decay = 0.8;                       % 回合温度调度：衰减系数（=1 表示每轮从 T0_round 全温度重启，各轮独立探索）
                                   % 若需轮间渐进降温，可改为 0.9~0.95；届时 T_min_round 作为下界生效
T_min_round = 2;                  % 回合温度调度：温度下界（T_decay=1 时此值不起作用，保留供调优用）
...
Value_Params.fig_size_belief  = fig_size_belief;
Value_Params.fig_size_alloc   = fig_size_alloc;
Value_Params.fig_size_inner   = fig_size_inner;
