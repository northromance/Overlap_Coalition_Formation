...
%  
%  功能说明：
%  1. 使用统一的场景（相同的SEED）初始化智能体、任务、资源
...
%
%  使用说明：
%  - 在 algorithms_to_run 中添加或注释要对比的算法
%  - 算法主函数应放在对应的 Com_XXX 文件夹中
%  - 每个算法应返回统一格式的 Value_data 和 history_data
% ========================================================================
...
% 算法选择
algorithms_to_run_ids = [1,4];  % 1=SA_Value, 2=贪心, 3=Huo2025, 4=Qi2023, 5=Qin2025
...
WORLD_ZMIN = 0;
WORLD_ZMAX = 0;
task_values = [800, 1000, 1500];  % 任务价值候选集（对应3种任务类型）
...
agent_fuel = 1;
agent_beta = 1;
min_resource_value = 2;         % 智能体资源最小值
max_resource_value = 4;         % 智能体资源最大值

% 任务资源需求参数（每种任务类型的资源需求范围）
task_type1_demand_max = 4;      % 类型1任务：低需求
task_type2_demand_max = 6;      % 类型2任务：中等需求
task_type3_demand_max = 8;      % 类型3任务：高需求
...
num_rounds = 50;                % 博弈轮数（SA算法专用；Huo算法会使用此参数，默认50）
resource_confidence = 0.7;      % 资源需求分位数置信度（SA算法专用）


% Qi2023算法效用函数参数
Qi_beta_m = 1.0;                % Sigmoid函数陡峭度参数
Qi_C_req = 0.5;                 % 需求阈值
Qi_omega = 0.1;                 % Sigmoid函数偏移参数
Qi_omega_1 = 1.0;               % 资源完成度权重
Qi_omega_2 = 0.01;              % 距离成本权重
Qi_omega_3 = 0.001;             % 能量损耗权重
